...
public:
    ...
    void render(const hittable& world, std::ostream& out) {
        ...
        // Render rows in parallel across available hardware threads. Each
        // thread claims the next unrendered row via an atomic counter, so
        // work stays balanced even though some rows (more reflective /
        // refractive bounces) cost more than others.
        unsigned int num_threads = std::thread::hardware_concurrency();
        if (num_threads == 0) num_threads = 4;
        num_threads = std::min<unsigned int>(num_threads, static_cast<unsigned int>(image_height));
